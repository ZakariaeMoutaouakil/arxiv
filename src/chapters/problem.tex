...
This property is particularly important when certifying robustness, as we need to ensure that our guarantees hold with high probability.
...
To derive a confidence interval, statisticians invert these concentration inequalities.
...
Other approaches have limitations, particularly in scenarios with low variance.
For example, Hoeffding's inequality assumes the worst-case scenario for variance, which is 1/4 for bounded random variables in [0,1].
...
\begin{proposition}[One-sided Empirical Bernstein Inequality]
    Let $X_1, X_2, \ldots, X_n$ be independent random variables with values in $[a,b]$, and let $\bar{X}_n = \frac{1}{n}\sum_{i=1}^n X_i$ be their empirical mean. Define the empirical variance as
    \[
